...
| Модалки сразу в HTML, `hidden` | Потом CSS/JS только показывают | Рисовать диалог картинкой |
...
6. Внизу `index.html` — два `role="dialog"` с `hidden`. Это заготовки этапов 4–5, не картинки.
7. `login.html` и `product.html` — запасной путь без JS.
